...
exports.createOrder = async (req, res) => {
    try {
        ...
        for (const item of items) {
            ...
            const menuItemId = typeof item === 'string' ? item : item.menuItem || item._id || item.menuItemId;
...
exports.getOrdersByCashierName = async (req, res) => {
    try {
        ...
        if (orders.length === 0) return res.status(404).json({ error: "No orders found for this cashier" })
        return res.status(200).json(orders)
    } catch (err) {
        console.error(err)
        return res.status(500).json({ error: "Server error" })
